...
public class SongSearchFragment extends LazyFragment implements OnItemClickListener{
	private EditText mEtSearch;
	private List<Music> mMusicList;
	private MusicAdapter mMusicAdapter;
	private MusicHomeActivity parentActivity;
	...
	protected void onViewAdded() {
		super.onViewAdded();
		parentActivity = (MusicHomeActivity ) getActivity();
	...
	private void requestMusicByName(){
		...
		if(StringUtils.isEmpty(name)){
			...
			IManager.getIMusic().getSearchMusicList(name,1);
			//缓冲列表
			showProgressDialog();
		}
	}
