...
public class AppDownloadManager {
	...
	// liujw
	public final static String COLUMN_NOTIFICATION_PACKAGE_NAME = "package_name";
	 ...
	 * completes.
	 */
	public final static String ACTION_DOWNLOAD_COMPLETE = "com.readboy.appstore.intent.action.DOWNLOAD_COMPLETE";
	 ...
	 * downloads UI.
	 */
	public final static String ACTION_NOTIFICATION_CLICKED = "com.readboy.appstore.intent.action.DOWNLOAD_NOTIFICATION_CLICKED";

	/**
	 * Intent action to launch an activity to display all downloads.
	 */
	public final static String ACTION_VIEW_DOWNLOADS = "com.readboy.appstore.intent.action.VIEW_DOWNLOADS";
	...
	public static final String[] UNDERLYING_COLUMNS = new String[] {
			...
			Downloads.Impl.COLUMN_NOTIFICATION_PACKAGE + " AS "
					+ COLUMN_NOTIFICATION_PACKAGE_NAME,
	...
	/**
	 * pause download ,add by liujw
	 * 
	 * @param ids
	 *            the IDs of the downloads to be paused
	 * @return the number of downloads actually pause
	 */
	...
	public int pauseAllDownload() {
		...
		return mResolver.update(mBaseUri, values, "("
				...
				+ Downloads.Impl.COLUMN_STATUS + " < ?)",
				new String[] { String.valueOf(100), String.valueOf(200) });
	}

	/**
	 * resume download ,added by liujw
	 * 
	 * @param ids
	 *            the IDs of downloads to be resumed
	...
	public int pauseDownload(String url) {
		...
		values.put(Downloads.Impl.COLUMN_STATUS,
				Downloads.Impl.STATUS_PAUSED_BY_APP);
		int result = mResolver.update(mBaseUri, values,
				Downloads.Impl.COLUMN_URI + " = ?", new String[] { url });
		if (result == 0)
			throw new RuntimeException("错误的暂停--不存在此任务");
		return result;
	}
	...
	public int resumeDownload(String url) {
		...
		values.put(Downloads.Impl.COLUMN_CONTROL, Downloads.Impl.CONTROL_RUN);
		values.put(Downloads.Impl.COLUMN_STATUS, Downloads.Impl.STATUS_RUNNING);
		return mResolver.update(mBaseUri, values, Downloads.Impl.COLUMN_URI
				+ " = ?", new String[] { url });
	}
	...
	public static class Request {
		 ...
		 * usuable only with
		 * {@link AppDownloadManager#addCompletedDownload(String, String, boolean, String, String, long, boolean)}
		 * .
		 */
		...
		/**
		 * liujw Set notiClass, to be used as destination when click downloading
		 * in download manager ui
		 * 
		 * @return this object
		 */
		...
		public Request setDestinationInExternalFilesDir(Context context,
			...
			setDestinationFromBase(mFile, subPath);
			return this;
		}
		 ...
		 * {@link AppDownloadManager#enqueue(Request)} is called.
		 */
		...
		ContentValues toContentValues(String packageName) {
			...
			// liujw
			putIfNonNull(values, Downloads.Impl.COLUMN_NOTIFICATION_CLASS,
					mNotiClass);
			putIfNonNull(values, Downloads.Impl.COLUMN_NOTIFICATION_EXTRAS,
					mNotiExtras);
			// end
	...
	static AppDownloadManager mDownloadManager;

	public static AppDownloadManager getDownloadManagerService(Context context) {
		if (mDownloadManager == null) {
			mDownloadManager = new AppDownloadManager(
	...
	public AppDownloadManager(ContentResolver resolver, String packageName) {
		mResolver = resolver;
	...
	/**
	 * @description 删除数据库中的记录 ，-1 表示想要的删除记录
	 * @param uri
	 *            下载的uri
	 * @param packageName
	 *            下载的应用程序包名
	 * */
	public int RowDeleted(String uri, String packageName) {
		if (uri != null && packageName != null) {
			String deleteWhere = Downloads.Impl.COLUMN_URI + " = " + "'" + uri
					+ "'" + " AND "
					+ Downloads.Impl.COLUMN_NOTIFICATION_PACKAGE + " = " + "'"
					+ packageName + "'";
			return mResolver.delete(mBaseUri, deleteWhere, null);
		}
		return -1;
	}
	...
	public Cursor queryDwonloadIWherePackageAndUri(String packageName,
			String uri) {
		String selection = Downloads.Impl.COLUMN_NOTIFICATION_PACKAGE + " = "
				+ "'" + packageName + "'" + " AND " + Downloads.Impl.COLUMN_URI
				+ " = " + "'" + uri + "'";
		Log.v("www", "selection===" + selection);
		Cursor cursor = mResolver.query(mBaseUri, UNDERLYING_COLUMNS,
				selection, null, null);
		if (null == cursor) {
			return null;
		}
	...
	public List<Integer> queryDownloadStates(String packageName, String url) {
		String selection = Downloads.Impl.COLUMN_NOTIFICATION_PACKAGE + " = '"
				+ packageName + "' AND " + Downloads.Impl.COLUMN_URI + " = '"
				+ url + "'";
		Cursor cursor = mResolver.query(mBaseUri,
				new String[] { Downloads.Impl.COLUMN_STATUS }, selection, null,
				null);
		if (cursor == null) {
			return null;
		}
	...
	public List<Integer[]> queryDownloadProgress(String packageName, String url) {
		String selection = Downloads.Impl.COLUMN_NOTIFICATION_PACKAGE + " = '"
				+ packageName + "' AND " + Downloads.Impl.COLUMN_URI + " = '"
				+ url + "'";
		Cursor cursor = mResolver.query(mBaseUri, new String[] {
				Downloads.Impl.COLUMN_CURRENT_BYTES,
				Downloads.Impl.COLUMN_TOTAL_BYTES }, selection, null, null);
		if (cursor == null) {
			return null;
		}
		List<Integer[]> is = new ArrayList<Integer[]>();
		try {
			...
			while (cursor.moveToNext()) {
				its = new Integer[2];
				its[0] = cursor.getInt(0);
				its[1] = cursor.getInt(1);
				is.add(its);
			}
		} finally {
			cursor.close();
	...
	public List<Long> queryDownloadID(String packageName, String url) {
		String selection = Downloads.Impl.COLUMN_NOTIFICATION_PACKAGE + " = '"
				+ packageName + "' AND " + Downloads.Impl.COLUMN_URI + " = '"
				+ url + "'";
		Cursor cursor = mResolver.query(mBaseUri,
				new String[] { Downloads.Impl._ID }, selection, null, null);
		if (cursor == null) {
			return null;
		}
	...
	public Cursor queryDownloadInfoWherePackage(String packageName) {
		String selection = Downloads.Impl.COLUMN_NOTIFICATION_PACKAGE + " = "
				+ "'" + packageName + "'";
		Log.v("www", "selection===" + selection);
		Log.v("www", "mBaseUri===" + mBaseUri);
		Log.v("www", "UNDERLYING_COLUMNS===" + UNDERLYING_COLUMNS);
		Cursor cursor = mResolver.query(mBaseUri, UNDERLYING_COLUMNS,
				selection, null, null);
		if (null == cursor) {
			return null;
	...
	public Uri getUriForDownloadedFile(long id) {
		...
		try {
			...
			if (cursor.moveToFirst()) {
				...
				if (AppDownloadManager.STATUS_SUCCESSFUL == status) {
					int indx = cursor
	 ...
	 * 
	 * @param context
	 *            the {@link Context} to use for accessing the
	 ...
	 * manager should transfer over a non-wifi connection. Over this size, the
	 * use will be warned, but will have the option to start the download over
	 * the mobile connection anyway.
	 * 
	 * @hide
	 */
	 ...
	 * @param description
	 *            the description that would appear for this file in Downloads
	 *            App.
	 ...
	 * @param path
	 *            absolute pathname to the file. The file should be
