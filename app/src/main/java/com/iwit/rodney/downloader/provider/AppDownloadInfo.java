...
import com.iwit.rodney.downloader.provider.Downloads.Impl;
 ...
 */
@TargetApi(Build.VERSION_CODES.ICE_CREAM_SANDWICH) public class AppDownloadInfo {
    ...
    public static class Reader {
        ...
        public AppDownloadInfo newDownloadInfo(Context context, SystemFacade systemFacade) {
        	AppDownloadInfo info = new AppDownloadInfo(context, systemFacade);
            updateFromDatabase(info);
            readRequestHeaders(info);
            return info;
        }

        public void updateFromDatabase(AppDownloadInfo info) {
        ...
        private void readRequestHeaders(AppDownloadInfo info) {
        ...
        private void addHeader(AppDownloadInfo info, String header, String value) {
    ...
    private boolean isReadyToStart(long now) {
        if (DownloadHandler.getInstance().hasDownloadInQueue(mId)) {
            // already running
        //	Log.e(TAG,"isReadyToStart alredy running ");
            return false;
        }
       ...
       //     Log.e(TAG,"isRedy");
        ...
        switch (mStatus) {
         ...
         //   	Log.e(TAG,"mStatus status running ");
                return true;
    ...
    private int checkIsNetworkTypeAllowed(int networkType) {
	    ...
	    //liujw 
	    if(mStatus == Downloads.Impl.STATUS_WAITING_FOR_NETWORK && flag != DownloadManager.Request.NETWORK_WIFI){
		return NETWORK_TYPE_DISALLOWED_BY_REQUESTOR ;
	    }
	   //end liujw
        }
